resource "cloudflare_ruleset" "http_request_cache_settings" {
  ...
  rules {
    ...
    enabled     = true
    expression  = "(http.host in {\"servarr.${var.domain_name}\" \"pihole.${var.domain_name}\" \"kvm.${var.domain_name}\" \"nzb.${var.domain_name}\" \"qbit.${var.domain_name}\" \"file.${var.domain_name}\" \"kvm-nl.${var.domain_name}\" \"sabnzbd.${var.domain_name}\" \"www.${var.domain_name}\" \"authentik.${var.domain_name}\" \"quantum.${var.domain_name}\"})"
  }
  rules {
    action = "set_cache_settings"
    action_parameters {
  ...
  # Removed: React App — no active DNS
  # Removed: Dillinger Cache Override — no active DNS
  rules {
    action = "set_cache_settings"
    action_parameters {
      cache = true
